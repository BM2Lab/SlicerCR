...
class CylindricalWall(xmlr.Object):
    def __init__(self, radius=0.0, thickness=0.0, height=0.0, angle=0.0):
        self.radius = radius
        self.thickness = thickness
        self.height = height
        self.angle = angle
        self.type = 'cylindrical_wall'


xmlr.reflect(CylindricalWall, tag='cylindrical_wall', params=[
    xmlr.Attribute('radius', float),
    xmlr.Attribute('thickness', float),
    xmlr.Attribute('height', float),
    xmlr.Attribute('angle', float)
])
...
class GeometricType(xmlr.ValueType):
    def __init__(self):
        self.factory = xmlr.FactoryType('geometric', {
            ...
            'sphere': Sphere,
            'mesh': Mesh,
            'cylindrical_wall': CylindricalWall
        })
...
#################################################################
####################Continuum Robot Extension####################
#################################################################
class ContinuumUnit(xmlr.Object):
    def __init__(self,radius=None, offset=None, angle=None, color=None, tag=None):
...
xmlr.reflect(ContinuumUnit, tag='continuum_unit', params=[
    
    xmlr.Attribute('radius', float),
    xmlr.Attribute('offset', float, False),
    xmlr.Attribute('angle', float, False),
    xmlr.Element('color', Color, False),
    xmlr.Attribute('tag', str, False),
])

class Backbone(xmlr.Object):
    def __init__(self, radius=None, initial_length=None, color=None):
        self.radius = radius
        self.initial_length = initial_length
        self.color = color
        self.trajectory = None

xmlr.reflect(Backbone, tag='backbone', params=[
    # xmlr.Element('radius', float),
    xmlr.Attribute('radius', float),
    xmlr.Attribute('initial_length', float),
    xmlr.Element('color', Color, False),
])

class Tendon(xmlr.Object):
    def __init__(self, id=None, radius=None, offset=None, 
                 angle=None, color=None,):
        self.id = id
        self.radius = radius
        self.offset = offset
        self.angle = angle
...
xmlr.reflect(Tendon, tag='tendon', params=[
    xmlr.Attribute('id', str),
    xmlr.Attribute('radius', float),
    xmlr.Attribute('offset', float),
    xmlr.Attribute('angle', float),
    xmlr.Element('color', Color, False)
])

class Tendons(xmlr.Object):
    def __init__(self, count=None):
        self.aggregate_init()
        self.count = count
        self.tendons = []

xmlr.reflect(Tendons, tag='tendons', params=[
    xmlr.Attribute('count', int),
    xmlr.AggregateElement('tendon', Tendon)
])

class Sheath(xmlr.Object):
    def __init__(self, radius=None, thickness=None, color=None, material=None):
        self.radius = radius
        self.color = color
        self.trajectory = None

xmlr.reflect(Sheath, tag='sheath', params=[
    xmlr.Attribute('radius', float),
    xmlr.Element('color', Color, False),
])
...
# Extend the existing Robot class to handle continuum segments
class ContinuumRobot(Robot):
    def __init__(self, name=None, version="1.0"):
        super(ContinuumRobot, self).__init__(name, version)
        self.segments = []
        self.segment_map = {}

    def add_aggregate(self, typeName, elem):
        # Call parent method first
        super(ContinuumRobot, self).add_aggregate(typeName, elem)
        
        # Handle segment-specific logic
        if typeName == 'segment':
            segment = elem
            self.segment_map[segment.name] = segment

    def add_segment(self, segment):
        self.add_aggregate('segment', segment)

    def get_segment_chain(self, root_segment, tip_segment):
        """Get chain of segments from root to tip"""
        chain = []
        current = tip_segment
        
        while current != root_segment:
            chain.append(current)
            # Find parent segment
            segment = self.segment_map.get(current)
            if segment and segment.parent_segment:
                current = segment.parent_segment
            else:
                break
                
        if current == root_segment:
            chain.append(root_segment)
            
        chain.reverse()
        return chain

# Update Robot reflection to include segments
xmlr.reflect(ContinuumRobot, tag='robot', params=[
    xmlr.Attribute('name', str),
    xmlr.Attribute('version', str, False),
    xmlr.AggregateElement('link', Link),
    xmlr.AggregateElement('joint', Joint),
    xmlr.AggregateElement('segment', Segment),  # Add segment support
    xmlr.AggregateElement('gazebo', xmlr.RawType()),
    xmlr.AggregateElement('transmission', 'transmission'),
    xmlr.AggregateElement('material', Material)
])
